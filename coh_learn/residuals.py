...
def mdl_resid(x=None,save_to=None,load_from=None,return_format=['distance'],
    ...
    Nmn=Nm-int(Nm*mdl.setup.pars['Nme'][0])
    Nfe=int(Nf*mdl.setup.pars['Nfe'][0])
    ...
    W=packed_data['W']
    dat=np.concatenate((hazm_d,hazs_d,hazd_d,everc_d,everm_d,flsc_d,flsm_d),axis=0)
    ...
    sim=np.concatenate((hazm_s,hazs_s,hazd_s,everc_s,everm_s,flsc_s,flsm_s),axis=0)
