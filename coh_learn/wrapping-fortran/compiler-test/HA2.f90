...
program VFI
	...
	r = 0.8*rmax+0.2*rmin ! initial value
    w = (1-alpha)*(alpha/(((1+tau)*r)+delta))**(alpha/(1-alpha))
	...
	do ii = 1,nstates
		do i = 1,gridlength
			do j = 1,gridlength
				UMAT(i,j,ii) = utility(agrid(i),agrid(j),L(ii),T)
			enddo
		enddo
	enddo
   ...
   ! enddo
    ...
    rnew = (1/(1+tau))*(alpha*((knew/lnew)**(alpha-1))-delta)
    
    change = abs(r-rnew)
    
    write(*,'(f8.6,x,f8.6,x,f8.6)') r, rnew, T 
	...
	T = tau*r*((alpha/((1+tau)*r + delta))**(1/(1-alpha)))*lnew
