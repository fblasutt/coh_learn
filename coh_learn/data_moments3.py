...
def dat_moments(sampling_number=5,weighting=False,covariances=False,relative=False,period=3,transform=1):   
    ...
    if weighting:   
        ...
        col=np.concatenate((hazmB,hazsB,hazdB,evercB,evermB,flscB,flsmB),axis=0)       
        dim=len(col)   
        ...
        #Compute optimal Weighting Matrix   
        col=np.concatenate((hazm,hazs,hazd,hazde,everc,everm,flsc,flsm),axis=0)       
        dim=len(col)   
        W=np.zeros((dim,dim))   
        for i in range(dim):   
                W[i,i]=1.0/col[i]**2  
        ...
        W=np.diag(np.ones(len(hazm)+len(hazs)+len(hazd)+len(everc)+len(everm)+len(flsc)+len(flsm)))#two is for fls+beta_unid   
