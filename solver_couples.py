...
def v_iter_couple(setup,dd,t,EV_tuple,edu,ushift,nbatch=nbatch_def,verbose=False,
    ...
    for ibatch in range(int(np.ceil(nexo/nbatch))):
        ...
        if np.min(money_t[0][None,:,None]+money_t[1][None,:,None]-(1-ls[il_opt_i])*wf[istart:ifinish][None,:,None]+money_t[2][None,:,None]-c_opt_i-x_opt_i)<-0.00001:
            print(222,np.min(money_t[0][None,:,None]+money_t[1][None,:,None]-(1-ls[il_opt_i])*wf[istart:ifinish][None,:,None]+money_t[2][None,:,None]-c_opt_i-x_opt_i))
    ...
    # #Ger labor supply
    # ilb=np.array(un + beta*EVc_all[:,:,:,0]<=uw + beta*EVc_all[:,:,:,1],dtype=np.int8)
    # assert np.all(ilb>0.5)
    # #Get everything now
